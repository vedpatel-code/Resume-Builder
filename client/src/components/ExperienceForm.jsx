...
const generateDescription = async (index) => {
    ...
    try {
        const { data } = await api.post('/api/ai/enhance-pro-job-desc', {userContent: prompt}, { headers: { Authorization: token}})
                    ...
                    {data.map((experience ,index)=>(
                        <div key={index} className="p-4 border border-gray-200 rounded-lg space-y-3">
                            ...
                            <div className='grid md:grid-cols-2 gap-2 items-center'>
                                ...
                                <input value={experience.start_date || ""} onChange={(e)=> updateExperience(index, "start_date", e.target.value)} type="month" className='px-3 py-2 text-sm rounded-lg' /> Staring date

                                <input value={experience.end_date || ""} onChange={(e)=> updateExperience(index, "end_date", e.target.value)} type="month" disabled={experience.is_current} className='px-3 py-2 text-sm rounded-lg disabled:bg-gray-100 ' /> Ending date
                            </div>
