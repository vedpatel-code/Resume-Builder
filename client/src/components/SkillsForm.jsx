  ...
  return (
    <div className='space-y-4'>
        ...
        <div className='flex gap-2'>
            <input type="text"placeholder='Enter a skill (e.g, Javascript,python)' className='flex-1 px-3py-2 text-sm' onChange={(e)=>setNewSkill(e.target.value)} value={newSkill}
        ...
        {data.length > 0 ? (
            <div className='flex flex-wrap gap-2'>
                {data.map((skill,index)=>(
                    <span key={index} className='flex items-center gap-1 px-3 py-1 bg-blue-100 tex-blue-800 rounded-full text-sm'>
                        {skill}
